...
// --- 3D BACKGROUND LOGIC ---
let camera; // Define camera in a broader scope for the scroll effect

function init3dBackground() {
    const scene = new THREE.Scene();
    // Initialize the camera defined outside this function
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('scroll', () => {
    // Check if the camera object from Three.js exists
    if (camera) {
      // Move the camera further away based on scroll position
      camera.position.z = 4 + window.scrollY * 0.005;
    }
  });
